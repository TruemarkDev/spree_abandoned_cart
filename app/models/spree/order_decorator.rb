  ...
  Order.class_eval do
    ...
    def self.eligible_abandoned_email_orders
      where('state != ?
            ...
            'complete',
            'paid',
            (Time.current - Spree::AbandonedCartEmailConfig::Config.email_timeframe_from),
            (Time.current - Spree::AbandonedCartEmailConfig::Config.email_timeframe_to)).joins(:line_items).distinct
    end
    ...
    def mark_abandoned_email_as_sent
      update_column :abandoned_email_sent_at, Time.current
    end
